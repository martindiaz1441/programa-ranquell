...
export default function Egresos() {
  ...
  const onImportExcel = (e) => {
    ...
    importarEgresosExcel(file, async (rows, erroresImport) => {
      ...
      if (!movimientos.length) {
        setResumen({ ok: 0, err: errores.length, errores });
        flash("No se guardaron egresos por errores");
        e.target.value = "";
        return;
      }
  ...
  return (
    <div className="app-container">
      ...
      <nav className="menu">
        ...
        <label
          style={{ background: "#0ea5e9", color: "#fff", borderRadius: 8, padding: "10px 14px", cursor: "pointer" }}
        >
          Importar Excel
      ...
      <div className="content">
                ...
                <tbody>
                  {filas.map((f, i) => (
                    <tr key={i}>
                      ...
                      <td>
                        <button type="button" className="btn-danger" onClick={() => handleRemove(i)}>
                          Quitar
                        </button>
                      </td>
                    </tr>
        ...
        {resumen && (
          <div className="card">
            ...
            <p>
              <b>OK:</b> {resumen.ok} &nbsp; | &nbsp; <b>Errores:</b> {resumen.err}
            </p>
            {resumen.errores?.length > 0 && (
              <ul className="bullets">{resumen.errores.map((e, idx) => <li key={idx}>{e}</li>)}</ul>
            )}
          </div>
        )}
